      ...
      <section className="py-24 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            ...
            {['ROOFING & WATERPROOFING', 'PROPERTY FINISHING & RENOVATION', 'CONSTRUCTION & MAINTENANCE'].map((category, catIndex) => (
              <div key={category} className="text-left">
                <motion.h2 
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  className="text-2xl md:text-3xl font-display font-bold mb-8 border-b border-white/10 pb-4"
                >
                  <span className="text-brand-green mr-2">//</span> {category}
                </motion.h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {servicesData.filter(s => s.category === category).map((service, index) => (
                    <motion.div
                      key={service.id}
                      initial={{ opacity: 0, y: 20 }}
                      whileInView={{ opacity: 1, y: 0 }}
